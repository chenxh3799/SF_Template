...
<sfform:form modelAttribute="filterMask" enctype="application/x-www-form-urlencoded; charset=UTF-8" action="<@jspEl 'ctx'/>${className}/main?_SF_SELECT_MODE=<@jspEl '_SF_SELECT_MODE'/>">
 ...
 <div class="content">
	...
	<table border="0" cellspacing="0" cellpadding="0" class="table_data">
		<tr>
	     ...
	     <div align="right">
			<button type="submit" id="search" class="btn" >搜索</button>	 
			<c:if test="<@jspEl 'not empty _SF_SELECT_MODE'/>">
				<input type="button" id="close" value="关闭" class="btn" onclick="javascript:window.close();"></input>
			</c:if>   
		    <c:if test="<@jspEl 'empty _SF_SELECT_MODE'/>">
		        <button type="button" id="addnew"  class="btn">创建</button>
			    <script type="text/javascript">
				    $("#addnew").click(function(){
					    window.location.href="<@jspEl 'ctx'/>${className}/new"});				
			    </script>
	       </c:if>			
	    </div>
