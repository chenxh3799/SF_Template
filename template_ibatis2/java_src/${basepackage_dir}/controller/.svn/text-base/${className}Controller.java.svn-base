...
public class ${className}Controller extends BaseSpringController{
     ...
     *进入索引列更新页面
	 **/
	 public ModelAndView editIndexColumns(HttpServletRequest request,HttpServletResponse response) throws Exception {
		<@generateIdParameter/>
		${className} ${classNameLower} = (${className})${classNameLower}Manager.getById(id);
		return new ModelAndView("/${className}/icEdit","${classNameLower}",${classNameLower});
	}
	
	/**
	 * 保存索引列更新对象
	 **/
	public ModelAndView updateIndexColumns(HttpServletRequest request,HttpServletResponse response) throws Exception {
		<@generateIdParameter/>
		
		${className} ${classNameLower} = (${className})${classNameLower}Manager.getById(id);
		bind(request,${classNameLower});
		${classNameLower}Manager.updateIndexs(${classNameLower});
		Flash.current().success(UPDATE_SUCCESS);
		return new ModelAndView(LIST_ACTION);
	}
